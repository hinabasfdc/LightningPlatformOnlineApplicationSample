...
export default class RelatedListEditor extends LightningElement {
  ...
  get fieldnameName(){ return NAME_FIELD['fieldApiName']; }
  get fieldnameDescription(){ return DESCRIPTION_FIELD['fieldApiName']; }
  get fieldnameCategory(){ return CATEGORY_FIELD['fieldApiName']; }
  get fieldnameStdColumnName(){ return STDCOLUMNNAME_FIELD['fieldApiName']; }
  get fieldnameDataType(){ return DATATYPE_FIELD['fieldApiName']; }
  get fieldnameRequired(){ return REQUIRED_FIELD['fieldApiName']; }
  get fieldnameOptions(){ return OPTIONS_FIELD['fieldApiName']; }
  get fieldnameValue(){ return VALUE_FIELD['fieldApiName']; }
  get fieldnameSortOrder(){ return SORTORDER_FIELD['fieldApiName']; }
  get fieldnameApplicationTemplate(){ return APPLICATIONTEMPLATE_FIELD['fieldApiName']; }
  ...
  **/
  ...
  wiredGetApplicationTemplateDetailRecordIds(value) {
    ...
    if (data) {
      ...
      // メニュー表示用にプロパティを追加
      for (let i = 0; i < array.length; i++) array[i]['DisplayName'] = '(' + array[i][this.fieldnameSortOrder] + ') ' + array[i][this.fieldnameName];
