...
export default class PanelCustomColumnData extends LightningElement {
  ...
  wiredGetApplicationCustomColumns({ data, error }) {
    if (data) {
      ...
      for (let i = 0; i < array.length; i++) {
        ...
        if (array[i][fnATD_DATATYPE_FIELD] === "数値")
          array[i][fnATD_ISNUMBER_FIELD] = true;
        else if (array[i][fnATD_DATATYPE_FIELD] === "日付")
          array[i][fnATD_ISDATE_FIELD] = true;
        else if (array[i][fnATD_DATATYPE_FIELD] === "時間")
          array[i][fnATD_ISTIME_FIELD] = true;
        else if (array[i][fnATD_DATATYPE_FIELD] === "チェックボックス")
          array[i][fnATD_ISCHECKBOX_FIELD] = true;
        else if (array[i][fnATD_DATATYPE_FIELD] === "通貨")
          array[i][fnATD_ISCURRENCY_FIELD] = true;
        else if (array[i][fnATD_DATATYPE_FIELD] === "ロングテキストエリア")
          array[i][fnATD_ISLONGTEXTAREA_FIELD] = true;
        else if (array[i][fnATD_DATATYPE_FIELD] === "URL")
          array[i][fnATD_ISLONGTEXTAREA_FIELD] = true;
        else if (array[i][fnATD_DATATYPE_FIELD] === "メール")
          array[i][fnATD_ISTEXT_FIELD] = true;
        else if (array[i][fnATD_DATATYPE_FIELD] === "選択リスト")
          array[i][fnATD_ISPICKLIST_FIELD] = true;
        else array[i][fnATD_ISTEXT_FIELD] = true;

        // チェックボックスの場合の画面表示切替え用
        if (array[i][fnATD_DATATYPE_FIELD] === "チェックボックス") {
          if (array[i][fnAD_TEXT_FIELD] === "true")
            array[i].isCheckboxChecked = true;
  ...
  handleChangeValue(evt) {
    ...
    } else if (datatype === "数値" || datatype === "通貨")
      this.recordApplicationDetails[idx][fnATD_ISNUMBER_FIELD] =
        evt.target.value;
    else if (datatype === "ロングテキストエリア")
      this.recordApplicationDetails[idx][fnAD_LONGTEXTAREA_FIELD] =
        evt.target.value;
  ...
  handleClickCancel() {
    // 退避させておいた値で上書き
    this.recordApplicationDetails = JSON.parse(
      this.previousRecordApplicationDetails
    );
    // 表示変更
  ...
  handleClickSave() {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (records[i][fnATD_ISNUMBER_FIELD])
        o[fnATD_ISNUMBER_FIELD] = records[i][fnATD_ISNUMBER_FIELD];
      else if (records[i][fnAD_LONGTEXTAREA_FIELD])
        o[fnAD_LONGTEXTAREA_FIELD] = records[i][fnAD_LONGTEXTAREA_FIELD];
